...
void renderSAMP ( void )
{
	...
	if ( !g_renderSAMP_initSAMPstructs )
	{
		...
		if (g_Dialog == nullptr || g_SAMP == nullptr)
			return;
		SetupHook(OnCloseDialog);
	...
	//	addMessageToChatWindow("%s%s %swith %s%s %s %sloaded (Доработка by %sMarko Friedmann%s)", szColorEnable, M0D_NAME, szColorText, szColorEnable, M0DM_NAME, M0DM_VERS, szColorText, szColorEnable, szColorText);
