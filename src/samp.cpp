...
struct stDialogInfo				*g_Dialog = nullptr;
DialogHandler					function = nullptr;
...
void ShowServerSampDialog(int dialogID, int typedialog, char *caption, char *text, char *button1, char *button2)
{
	showSampDialog(1, dialogID, typedialog, caption, text, button1, button2);
}

void ShowLocalSampDialog(int dialogID, int typedialog, char *caption, char *text, char *button1, char *button2)
{
	showSampDialog(0, dialogID, typedialog, caption, text, button1, button2);
}
...
void _declspec (naked) CloseDialogHook(void)
{
	...
	__asm pushad
	static DWORD dwTemp;
	if (g_SAMP != nullptr && g_Dialog != nullptr && function != nullptr && iButtonID && !g_Dialog->bServerside)
	{
		CreateThread(0, 0, (LPTHREAD_START_ROUTINE)function, (LPVOID)g_Dialog, 0, 0);
	}
	dwTemp = g_dwSAMP_Addr + SAMP_DIALOG_CLOSE + 6;
	__asm popad
...
void SetupHook(DialogHandler func)
{
	if (func == nullptr)
		return;
	function = func;

	uint32_t addr = g_dwSAMP_Addr + SAMP_DIALOG_CLOSE;
	DWORD dwoldprot = SetProtection(g_dwSAMP_Addr + SAMP_DIALOG_CLOSE, PAGE_EXECUTE_READWRITE);
