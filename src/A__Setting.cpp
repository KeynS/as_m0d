...
void initAdminSettings()
{
    ...
    A_Set.keycombo = {
		...
		std::make_pair("stop", keycombo{ 2, { '0' } }),
    };
    ...
    if (A_Set.bLogBan)
        A_Set.fLogBan = fopen(".\\mod_sa\\Logs\\ban.log", "a");
    if (A_Set.bLogWarn)
        A_Set.fLogWarn = fopen(".\\mod_sa\\Logs\\warn.log", "a");
    if (A_Set.bLogKillList)
        A_Set.fLogKillList = fopen(".\\mod_sa\\Logs\\KillList.log", "a");
}

/*void removeLine(const char *filename, size_t index)
{
...
{
	std::string tempFilename(filename); tempFilename.append(".tmp");
	std::ifstream in(filename);
	std::ofstream out(tempFilename, std::ios::trunc | std::ios::out);
...
{
	std::ofstream out(".\\" M0D_FOLDER "Checkers\\Players.ini", std::ios::app);
	if (out.is_open()) {
		out << "\nNick=[" << nickname << "] Reason=[" << reason << ']';
		out.close();
	}
}
...
void getAdminList()
{
	std::ifstream fileAdmins(".\\" M0D_FOLDER "Checkers\\Admins.ini", std::ios::in);
	if (fileAdmins.is_open()) {
		std::string adminName;
		while (std::getline(fileAdmins, adminName)) {
			if (!adminName.empty())
...
void getPlayerList()
{
	std::ifstream filePlayers(".\\" M0D_FOLDER "Checkers\\Players.ini", std::ios::in);
