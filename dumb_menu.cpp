	...
	case MENU_OP_INC:
		...
		case ID_FONT_TAGS:
			if ((mod == -1 && A_Set.tags > 1) || (mod == 1 && A_Set.tags < 4)) {
				...
				menu_item_name_set(item, "Tags  %hhu", A_Set.tags);
			}
			break;
		}
...
void menu_maybe_init ( void )
{
	...
	*menu_color_feedback_setting, *menu_color_support_setting, *menu_color_trace_setting, *menu_color_trace_hit_setting;
	...
	menu_item_add(menu_admin_trace, menu_color_trace_setting, "Color trace", ID_NONE, MENU_COLOR_DEFAULT, NULL);
	...
	menu_item_add(menu_admin_trace, menu_color_trace_hit_setting, "Color hit trace", ID_NONE, MENU_COLOR_DEFAULT, NULL);
	...
	snprintf(name, sizeof(name), "Tags  %hhu", A_Set.tags);
